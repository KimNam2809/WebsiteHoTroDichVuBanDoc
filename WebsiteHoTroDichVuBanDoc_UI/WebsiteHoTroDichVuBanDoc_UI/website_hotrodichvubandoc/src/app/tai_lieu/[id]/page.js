...
import { ArrowLeft, BookOpen, Calendar, User, Info, Hash, Clock, CheckCircle, Heart, Share2 } from 'lucide-react';

const FASTAPI_URL = process.env.FASTAPI_BACKEND_URL;

// === 1. Hàm Helper gọi API (Có hỗ trợ Auth) ===
async function fetchFromAPI(endpoint, options = {}) {
    const cookieStore = await cookies();
    const token = cookieStore.get('auth_token')?.value;

    const cacheOption = token ? 'no-store' : 'force-cache';
    const nextOption = token ? {} : { revalidate: 60 };
    ...
    try {
        const res = await fetch(`${FASTAPI_URL}${endpoint}`, {
            headers,
            cache: options.cache || cacheOption,
...
// === 2. Lấy danh sách ID sách TÔI đang mượn ===
async function getMyBorrowedCopyIds() {
    try {
        const [borrowing, overdue] = await Promise.all([
            fetchFromAPI('/api/v1/muon-tra/danh-sach-chi-tiet-muon-tra?trang_thai=daMuon'),
            fetchFromAPI('/api/v1/muon-tra/danh-sach-chi-tiet-muon-tra?trang_thai=quaHan')
        ]);

        const safeBorrowing = Array.isArray(borrowing) ? borrowing : [];
        const safeOverdue = Array.isArray(overdue) ? overdue : [];
        const allLoans = [...safeBorrowing, ...safeOverdue];

        return allLoans.map(loan => {
            const id = loan.mabansao || loan.maBanSao || loan.ma_ban_sao;
            return String(id);
...
// === 3. Lấy danh sách ID sách TÔI ĐÃ ĐẶT TRƯỚC ===
async function getMyReservedCopyIds() {
    try {
        const res = await fetchFromAPI('/api/v1/dat-truoc/');

        // Kiểm tra dữ liệu trả về là Mảng hay Object chứa data
        const reservations = Array.isArray(res) ? res : (res?.data || []);
...
// === 4. Lấy dữ liệu chi tiết tác phẩm ===
...
// === 5. Component Trang Chi Tiết ===
export default async function ChiTietTacPhamPage({ params }) {
    const { id } = await params;

    const [details, myBorrowedIds, myReservedIds] = await Promise.all([
        getWorkDetails(id),
        getMyBorrowedCopyIds(),
        getMyReservedCopyIds() // Gọi hàm mới
    ]);

    const { work, categories, copies, availableCount, error } = details;
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-20">
                <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100 flex flex-col md:flex-row">
                    ...
                    <div className="md:w-1/3 lg:w-[30%] bg-gray-50 p-8 flex flex-col items-center border-b md:border-b-0 md:border-r border-gray-100">
                        <div className="relative w-48 md:w-56 aspect-2/3 shadow-2xl rounded-lg overflow-hidden transform -translate-y-12 md:translate-y-0 hover:scale-105 transition-transform duration-500 z-10 mb-6 group bg-gray-200">
                            {work.anhbia ? (
                                <Image src={work.anhbia} alt={work.tentacpham} fill className="object-cover" sizes="(max-width: 768px) 100vw, 300px" priority />
                            ) : (
                                <div className="w-full h-full flex flex-col items-center justify-center text-gray-400"><BookOpen size={48} strokeWidth={1} /></div>
                            )}
                        </div>
                    ...
                    <div className="md:w-2/3 lg:w-[70%] p-8 md:p-10 flex flex-col">
                        <div className={`mb-8 p-4 rounded-xl flex items-start sm:items-center justify-between border ${availableCount > 0 ? 'bg-green-50 border-green-100' : 'bg-orange-50 border-orange-100'}`}>
                            <div className="flex gap-3">
                                ...
                                <div>
                                    ...
                                    <p className={`text-sm ${availableCount > 0 ? 'text-green-600' : 'text-orange-600'}`}>{availableCount > 0 ? `Hiện còn ${availableCount} bản tại thư viện. Đăng ký mượn ngay!` : 'Vui lòng quay lại sau hoặc đặt lịch hẹn trước.'}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Truyền các props xuống Client Component */}
