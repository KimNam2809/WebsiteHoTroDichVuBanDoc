...
export default function ThongBaoPage() {
    ...
    const handleSelect = async (notif) => {
        ...
        if (notif.trangthai !== 'daXem') {
            const success = await markAsReadAction(notif.mathongbao);
            if (success) {
                // Update local state
                                ...
                                {notifications.map((notif) => (
                                    <li
                                        ...
                                        className={`p-4 cursor-pointer hover:bg-blue-50 transition-colors relative group ${selectedNotif?.mathongbao === notif.mathongbao ? 'bg-blue-50 ring-inset ring-2 ring-blue-500' : ''
                                            } ${notif.trangthai !== 'daXem' ? 'bg-white' : 'bg-gray-50/50'}`}
                                    >
                                        <div className="flex justify-between items-start mb-1">
                                            <h3 className={`font-semibold line-clamp-1 ${notif.trangthai !== 'daXem' ? 'text-gray-900' : 'text-gray-600 font-medium'}`}>
                                                {notif.tieude}
                                            </h3>
