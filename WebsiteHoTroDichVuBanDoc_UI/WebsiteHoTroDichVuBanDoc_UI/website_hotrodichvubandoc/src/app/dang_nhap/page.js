...
// === 1. Component Nút Google ===
function GoogleLoginButton() {
    const [isLoading, setIsLoading] = useState(false);

    const handleGoogleLogin = async () => {
        try {
            console.log("Bắt đầu đăng nhập Google..."); // Debug log
            setIsLoading(true);

            // Tạo client ngay tại trình duyệt
            ...
            const { error } = await supabase.auth.signInWithOAuth({
                provider: 'google',
                options: {
                    // Chuyển hướng về route callback
            ...
            // Nếu thành công, trang sẽ tự chuyển hướng, không cần set loading false
        } catch (err) {
    ...
    return (
        <button
            ...
            disabled={isLoading}
            className="w-full py-3 px-4 border border-gray-200 bg-white text-gray-700 rounded-xl font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
        >
...
const LoginForm = () => {
    ...
    return (
        <form action={formAction} className="space-y-5 animate-in">
            ...
            {/* Sử dụng Component Google Login đã sửa */}
            <div className="pt-2">
                <GoogleLoginButton />
            </div>
        </form>
...
const RegisterForm = () => {
    ...
    const handleRegister = async (e) => {
        ...
        // 👇 Sử dụng createClient được import
        const supabase = createSupabaseClient();
        ...
        try {
            ...
            if (error) {
                ...
                setMessage({ type: 'error', text: 'Email này đã tồn tại.' });
            } else {
                setMessage({ type: 'success', text: 'Đăng ký thành công! Hãy kiểm tra Email để xác nhận.' });
                e.target.reset();
            }
        } catch (err) {
        ...
        <form onSubmit={handleRegister} className="space-y-5 animate-in">
            ...
            <div className="space-y-1">
                ...
                <div className="relative group">
                    ...
                    <input name="fullname" type="text" required className="block w-full pl-10 pr-3 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none text-sm" placeholder="Nguyễn Văn A" />
                </div>
            </div>
            ...
            <div className="space-y-1">
                ...
                <div className="relative group">
                    ...
                    <input name="email" type="email" required className="block w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition-all" placeholder="email@example.com" />
                </div>
            </div>
            ...
            <div className="space-y-1">
                ...
                <div className="relative group">
                    ...
                    <input name="password" type="password" required minLength={6} className="block w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition-all" placeholder="••••••••" />
                </div>
            </div>

            {message && (
                <div className={`p-3 rounded-lg text-sm font-medium border flex items-center gap-2 ${message.type === 'success' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-600'}`}>
                    {message.type === 'success' ? '✅' : '⚠️'} {message.text}
                </div>
            )}

            <button disabled={loading} className="w-full py-3.5 px-4 bg-linear-to-r from-green-500 to-emerald-600 text-white rounded-xl font-bold text-lg shadow-lg shadow-green-500/30 hover:shadow-green-500/50 transition-all flex items-center justify-center gap-2 disabled:opacity-70">
                ...
                <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-white/20">
                    ...
                    <div className="flex p-1.5 mx-8 mt-6 bg-gray-100/80 rounded-xl">
                        <button
                            ...
                            className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-300 ${view === 'login' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                        >
                            Đăng nhập
                        </button>
                        <button
                            onClick={() => setView('register')}
                            className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-300 ${view === 'register' ? 'bg-white text-green-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                        >
                            Đăng ký
                        </button>
                    </div>
