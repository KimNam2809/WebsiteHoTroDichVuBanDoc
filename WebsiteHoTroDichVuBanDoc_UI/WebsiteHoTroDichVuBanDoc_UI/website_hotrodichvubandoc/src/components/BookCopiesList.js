...
export default function BookCopiesList({ copies, initialOwnedIds = [], initialReservedIds = [] }) {
    ...
    const [myReservedCopyIds, setMyReservedCopyIds] = useState(new Map(
        initialReservedIds.map(item => [item.id, item.status])
    ));
    ...
    const handleBorrow = async () => {
        ...
        if (result.success) {
            ...
            // Clear reservation status (optional, but good for UI)
            setMyReservedCopyIds(prev => {
                const newMap = new Map(prev);
                newMap.delete(String(selectedCopy.mabansao));
                return newMap;
            });
        } else {
    ...
    const handleReserve = async () => {
        ...
        if (result.success) {
            ...
            // Cập nhật UI ngay lập tức: Thêm ID vào danh sách đã đặt với trạng thái 'kichHoat'
            setMyReservedCopyIds(prev => new Map(prev).set(String(selectedCopy.mabansao), 'kichHoat'));
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                            {copies.length === 0 ? (
                                ...
                                copies.map((copy) => {
                                    ...
                                    // Logic mới cho Đặt trước: Kiểm tra trạng thái
                                    const reservationStatus = myReservedCopyIds.get(copyIdStr);
                                    const isReserved = reservationStatus === 'kichHoat';
                                    const isReadyToBorrow = reservationStatus === 'hoanThanh';
